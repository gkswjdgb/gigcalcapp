...
const FAQ_DATA: any = {
  profit: [
    { q: 'How do I calculate true profit for Uber Eats and DoorDash?', a: "To calculate true profit (Net Income), you must subtract operating costs from your total app payout. This calculator deducts gas and vehicle depreciation (wear & tear) to show your 'Real Hourly Wage', which is often 30-40% lower than the gross revenue shown in the app." },
    { q: 'What is a good profit per mile for delivery drivers?', a: "Most experienced gig drivers aim for at least $1.50 to $2.00 per mile. If your 'Real $/Mile' is below $1.00, you might be losing money after vehicle expenses." }
  ],
  safe: [
    { q: 'How much should gig workers save for taxes?', a: 'As an independent contractor (1099), taxes are not withheld. It is recommended to save 15-20% of your Net Profit (after mileage deduction) for quarterly taxes. This tool automatically calculates that safe amount for you.' },
    { q: 'Why do I need a sinking fund for car repairs?', a: "Gig work puts heavy stress on your vehicle. Saving about 5-8 cents per mile creates a 'Sinking Fund' for inevitable costs like tires, brakes, and oil changes, preventing future debt." }
  ],
  tax: [
    { q: 'What is the 2024 IRS Standard Mileage Rate?', a: 'For 2024, the IRS Standard Mileage Rate is 67 cents per mile for business use. This deduction significantly lowers your taxable income. For example, driving 1,000 miles reduces your taxable income by $670.' },
    { q: 'Does the mileage deduction count as cash?', a: "No, it's a 'Tax Deduction', not a cash reimbursement. However, it acts as a 'Tax Shield' by lowering the amount of income you owe taxes on, effectively keeping more money in your pocket." }
  ],
  goal: [
    { q: 'How many hours do I need to drive to earn $500?', a: "It depends on your 'Real Hourly Wage'. If your net profit is $20/hr, you need 25 hours. This calculator uses your actual real-time earnings to give you a precise work-hour target." }
  ],
  house: [
    { q: 'Can I qualify for a mortgage as a Gig Worker?', a: "Yes, but lenders look at your 'Net Profit' on Schedule C, not your gross earnings. Use this calculator to see if your actual take-home pay can cover estimated mortgage payments and property taxes." }
  ]
};
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-fade-in">
      <div className="bg-white rounded-3xl w-full max-w-sm p-6 shadow-2xl relative">
        ...
        <p className="text-xs text-slate-500 mb-4">Found a bug? Have a suggestion? Let us know!</p>
        ...
        <p className="text-[10px] text-slate-400 mt-3 text-center">Your email is kept private.</p>
      </div>
    </div>
  );
};
...
const DisclaimerFooter = () => (
  <div className="px-6 py-8 bg-slate-100 text-center border-t border-slate-200">
    <p className="text-[10px] text-slate-400 leading-relaxed"><strong>DISCLAIMER:</strong> This tool is for informational purposes only. Estimates vary by location. Consult a professional.</p>
...
export default function Page() {
  ...
  useEffect(() => {
    ...
    if (mi > 0) setProfitPerMile(parseFloat((net / mi).toFixed(2))); else setProfitPerMile(0);
    if (ord > 0) setPayPerOrder(parseFloat((inc / ord).toFixed(2))); else setPayPerOrder(0);
    ...
    let mortgage = 0;
    if (monthlyRate > 0) mortgage = (loanPrincipal * (monthlyRate * Math.pow(1 + monthlyRate, numberOfPayments))) / (Math.pow(1 + monthlyRate, numberOfPayments) - 1);
    ...
    const safeAmount = net - repairFund - taxFund - emergencyFund - vacationFund;
              ...
              <div className="animate-fade-in-up">
                ...
                <div className="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm space-y-3">
                  ...
                  <div className="space-y-3">
                    ...
                    {useVacation && (<div className="flex items-center justify-between"><div className="flex items-center gap-2"><span className="w-2 h-2 rounded-full bg-purple-400"></span><div><p className="text-xs font-bold text-slate-700">Vacation Fund</p><p className="text-[10px] text-slate-400">My PTO ({saveVacationRate}%)</p></div></div><span className="text-sm font-bold text-purple-500">-${vacationSavings.toFixed(2)}</span></div>)}
                  </div>
                </div>
              ...
              <div className="animate-fade-in-up">
                ...
                <div className="space-y-6">
                  ...
                  <div className={`p-6 rounded-3xl shadow-xl transition-colors ${isRenting ? cashFlow >= 0 ? 'bg-emerald-500 text-white' : 'bg-red-500 text-white' : 'bg-slate-900 text-white'}`}>
                    ...
                    {isRenting && (<><p className="text-white/80 text-[10px] font-bold uppercase tracking-widest mb-1">Monthly Cash Flow</p><div className="text-5xl font-black tracking-tighter mb-2">{cashFlow >= 0 ? '+' : ''}${cashFlow.toFixed(0)}</div><p className="text-xs font-medium text-white/90 mb-4">{cashFlow >= 0 ? 'You make money every month!' : 'You lose money every month.'}</p></>)}
                  </div>
                  <div className="bg-white border border-emerald-100 p-5 rounded-3xl shadow-sm relative overflow-hidden"><div className="relative z-10"><div className="flex items-center gap-2 mb-2"><div className="bg-emerald-100 p-1.5 rounded-full text-emerald-600"><Icons.Shield /></div><span className="text-xs font-bold text-slate-500 uppercase">Yearly Tax Benefit</span></div><div className="text-3xl font-black text-slate-900">${interestDeduction.toFixed(0)}</div><p className="text-[10px] text-slate-400 mt-1 leading-tight">Estimated Interest Deduction (Year 1).</p></div></div>
                </div>
                <AdSlot />
              </div>
            )}
